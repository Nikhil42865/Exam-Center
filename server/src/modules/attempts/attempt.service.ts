...
import { AttemptModel, IAttempt, IAttemptSnapshotQuestion } from "./attempt.model.js";
...
export class AttemptService {
  async startOrResumeAttempt(examId: string, userId: string): Promise<ActiveAttemptDto> {
    const exam = await ExamModel.findById(examId);
    if (!exam || exam.status !== "published") {
      throw AppError.badRequest("Exam is not available");
    }

    const subject = await SubjectModel.findOne({ _id: exam.subjectId, isActive: true });
    if (!subject) {
      throw AppError.badRequest("Subject is not currently active");
    }

    const now = new Date();
    if (exam.availableFrom && now < exam.availableFrom) {
      throw AppError.badRequest("Exam availability period has not started yet");
    }
    if (exam.availableUntil && now > exam.availableUntil) {
      throw AppError.badRequest("Exam availability period has ended");
    }

    // Check for existing active attempt (FR-ATTEMPT-03: Only one in_progress attempt per candidate per exam)
    const existingActive = await AttemptModel.findOne({
      examId,
      userId,
      status: "in_progress",
    });
    ...
    // Check attempt limit
    const pastAttemptsCount = await AttemptModel.countDocuments({
      examId,
      userId,
    ...
    if (pastAttemptsCount >= exam.attemptLimit) {
      throw AppError.badRequest("You have reached the maximum allowed attempts for this exam");
    }

    // Load questions to create immutable snapshot (FR-ATTEMPT-05)
    let questions = await QuestionModel.find({ examId }).sort({ order: 1 });
    if (questions.length === 0) {
      throw AppError.badRequest("Exam has no questions");
    }
    ...
    const snapshotQuestions: IAttemptSnapshotQuestion[] = questions.map((q, idx) => {
  ...
  async getActiveAttempt(attemptId: string, userId: string): Promise<ActiveAttemptDto> {
    ...
    if (attempt.userId.toString() !== userId) {
      throw AppError.forbidden("You do not have permission to access this attempt");
    }

    const now = new Date();
    if (attempt.status === "in_progress" && now > attempt.expiresAt) {
      await this.autoFinalizeExpiredAttempt(attempt);
      throw AppError.badRequest("Time for this attempt has expired and it was automatically submitted.");
    }

    if (attempt.status !== "in_progress") {
      throw AppError.badRequest("This attempt is already submitted.");
    }
  ...
  async saveAnswer(
    ...
    if (attempt.status !== "in_progress" || now > attempt.expiresAt) {
      ...
      throw AppError.badRequest("Cannot update answer: attempt has expired or already submitted");
    }

    // Verify question belongs to attempt snapshot
    const question = attempt.snapshot.questions.find((q) => q.questionId === questionId);
    if (!question) {
      throw AppError.badRequest("Question does not belong to this exam attempt");
    }
    ...
    if (dto.selectedOptionId && !question.options.some((o) => o.id === dto.selectedOptionId)) {
      throw AppError.badRequest("Selected option does not belong to this question");
    }
  ...
  async submitAttempt(attemptId: string, userId: string): Promise<ResultSummaryDto> {
    const attempt = await AttemptModel.findById(attemptId);
    if (!attempt) {
      throw AppError.notFound("Attempt not found");
    }
    ...
    const gradeResult = gradingService.grade(
      attempt.snapshot.questions,
      attempt.answers,
      attempt.snapshot.passingPercentage
    );
  ...
  async autoFinalizeExpiredAttempt(attempt: IAttempt): Promise<void> {
  ...
  async getResult(attemptId: string, userId: string): Promise<DetailedReviewDto> {
    const attempt = await AttemptModel.findById(attemptId);
    if (!attempt) {
      throw AppError.notFound("Attempt not found");
    }
    ...
    if (!showScore) {
      throw AppError.badRequest("Score results are not visible for this exam according to policy.");
    }
  ...
  async getCandidateHistory(userId: string): Promise<AttemptHistoryItemDto[]> {
