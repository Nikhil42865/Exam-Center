...
class ExamService {
    async createExam(data, userId) {
        ...
        if (data.slug) {
            ...
            if (existing) {
                throw errors_js_1.AppError.conflict(`An exam with the slug '${slug}' already exists. Please choose a different slug.`);
            }
        }
        else {
            let uniqueSlug = slug;
            let counter = 1;
            while (await exam_model_js_1.ExamModel.findOne({ slug: uniqueSlug })) {
                counter++;
